...
def connect_db():
    conn = psycopg2.connect(
        host="localhost",
        port=5432,
        database="postgres",  # Burası postgres kalıyor, çünkü veri tabanın bu.
        user="postgres",
        password=password
    )
    # Python'a direkt küçük harfli dsdb şemasına bakmasını söylüyoruz:
    conn.cursor().execute("SET search_path TO dsdb;") 
    return conn
